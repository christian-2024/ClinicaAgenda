...
namespace ClinAgenda.src.Core.Interfaces
{
    ...
    {
        Task<(int total, IEnumerable<AppointmentListDTO> appointments)> GetAppointmentAsync(string? patientName, string? doctorName, int? specialtyId, int itemsPerPage, int page);
        Task<int> InsertAppointmentAsync(AppointmentDTO appointment);
        Task<AppointmentDTO?> GetByIdAsync(int id);
        Task<bool> UpdateAsync(AppointmentInsertDTO appointment);
        Task<int> DeleteAsync(int appointment);
    }
}